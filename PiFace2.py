...
vs = VideoStream(usePiCamera=True, resolution=resolution, framerate=30).start()
time.sleep(2.0)
...
def factor_3(this_card_pin):
    ...
    while True:
        ...
        if process_this_frame:
            # Scale the frame down to 25% size for perfomance
        ...
        for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            cv2.putText(frame, textname, (left, y), cv2.FONT_HERSHEY_SIMPLEX,
                .8, (0, 255, 255), 2)
