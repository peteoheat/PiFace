...
def main():
    ...
    video_process = FaceRecognitionProcess(capture=video_capture,
                                           known_encodings=knownfacedata["encodings"],
                                           known_names=knownfacedata["names"],
                                           fx=scale_factor,
    ...
    while True:
        ...
        for (left, top, right, bottom), name in zip(locations, names):
            ...
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 180, 180), 2)

            # Draw a label with a name
            #cv2.rectangle(frame, (left-2, top - 35), (right+2, top), (244, 134, 66), cv2.FILLED)

            font = cv2.FONT_HERSHEY_DUPLEX
            cv2.putText(frame, name, (left + 6, top - 6), font, 1.0, (0, 180, 180), 2)
